...
function AddonsPage({ addons, installed, available, query, selected, environment, environmentError, isUninstalling, isRefreshing, installingAddonId, setQuery, setSelected, install, onRefresh, onOpenPermissions, onUninstall }: { addons: Addon[]; installed: Addon[]; available: Addon[]; query: string; selected: string[]; environment: EnvironmentReport | null; environmentError: string | null; isUninstalling: boolean; isRefreshing: boolean; installingAddonId: string | null; setQuery: (value: string) => void; setSelected: (value: string[]) => void; install: (id: string) => void; onRefresh: () => void; onOpenPermissions: () => void; onUninstall: () => void }) {
  ...
  return <><header className="flex items-start justify-between gap-6"><div><p className="text-xs font-bold tracking-[0.12em] text-brand-600">WPS 插件</p><h2 className="mt-1 text-3xl font-bold tracking-tight">插件</h2><p className="mt-2 text-sm text-slate-500 dark:text-slate-400">管理 WPS 中的加载项</p></div><button type="button" onClick={onRefresh} disabled={isRefreshing || Boolean(installingAddonId) || isUninstalling} aria-busy={isRefreshing} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm font-bold text-slate-700 transition hover:border-brand-200 hover:text-brand-700 disabled:cursor-wait disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"><ArrowClockwiseIcon size={18} className={isRefreshing ? "animate-spin" : ""} />{isRefreshing ? "刷新中…" : "刷新"}</button></header>
    ...
    <section className="mt-8"><div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-3"><h3 className="text-lg font-bold">已安装</h3><span className="text-xs font-medium text-slate-400">选中可卸载</span></div><span className="text-sm text-slate-500">{installed.length} 个插件</span></div><div className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"><div className="grid grid-cols-[44px_minmax(210px,1.8fr)_100px_140px_110px] items-center border-b border-slate-100 px-4 py-3 text-xs font-bold text-slate-400 dark:border-slate-800"><label className="grid size-9 cursor-pointer place-items-center"><input className="size-5 cursor-pointer accent-brand-600 disabled:cursor-not-allowed" aria-label="选择全部已安装插件" disabled={isUninstalling} type="checkbox" checked={installed.length > 0 && selected.length === installed.length} onChange={() => setSelected(selected.length === installed.length ? [] : installed.map((item) => item.id))} /></label><span>名称</span><span>版本</span><span>来源</span><span>状态</span></div>{installed.map((addon) => <div key={addon.id} className="grid grid-cols-[44px_minmax(210px,1.8fr)_100px_140px_110px] items-center px-4 py-4 text-sm transition hover:bg-slate-50 dark:hover:bg-slate-800/50"><label className="grid size-9 cursor-pointer place-items-center"><input className="size-5 cursor-pointer accent-brand-600 disabled:cursor-not-allowed" aria-label={`选择${addon.name}`} disabled={isUninstalling} type="checkbox" checked={selected.includes(addon.id)} onChange={() => toggle(addon.id)} /></label><span className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-brand-50 text-brand-600 dark:bg-brand-600/15"><FileTextIcon size={21} /></span><span><strong className="block">{addon.name}</strong><small className="mt-0.5 block text-slate-500">{addon.description}</small></span></span><span>{addon.version}</span><span className="text-slate-500 dark:text-slate-400">{addon.source}</span><span><StatusPill tone="success"><span className="size-1.5 rounded-full bg-emerald-500" />{addon.health}</StatusPill></span></div>)}</div>{selected.length > 0 && <div className="mt-3 flex items-center justify-between rounded-xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-700 dark:bg-slate-900"><span className="text-sm font-semibold">{isUninstalling ? "正在卸载所选插件…" : `已选择 ${selected.length} 项`}</span><button type="button" disabled={isUninstalling} onClick={onUninstall} className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-bold text-rose-600 hover:bg-rose-50 disabled:cursor-wait disabled:opacity-60 dark:hover:bg-rose-950/30"><TrashIcon size={18} />卸载</button></div>}</section>
    <section className="mt-9"><h3 className="mb-3 text-lg font-bold">可安装与更新</h3><div className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">{available.length ? available.map((addon) => { const isInstalling = installingAddonId === addon.id; const installedVersion = installed.find((item) => item.id === addon.id)?.version; const isUpdate = installedVersion !== undefined; return <div key={addon.id} className="flex items-center gap-4 px-5 py-5"><span className="grid size-11 place-items-center rounded-xl bg-brand-50 text-brand-600 dark:bg-brand-600/15"><CalendarDotsIcon size={24} /></span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h4 className="font-bold">{addon.name}</h4><span className="text-xs text-slate-400">v{addon.version}</span>{isUpdate && <StatusPill tone="warning">可更新</StatusPill>}</div><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{addon.description}</p><p className="mt-2 text-xs font-medium text-brand-600 dark:text-brand-300">{isUpdate ? `已安装 v${installedVersion} · ${addon.source}` : addon.source}</p></div><button type="button" disabled={isRefreshing || Boolean(installingAddonId)} onClick={() => install(addon.id)} className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-brand-600/20 transition hover:bg-brand-700 disabled:cursor-wait disabled:opacity-70">{isInstalling ? <ArrowClockwiseIcon size={18} className="animate-spin" /> : <DownloadSimpleIcon size={18} />}{isInstalling ? (isUpdate ? "正在更新…" : "正在安装…") : (isUpdate ? "更新" : "安装")}</button></div>; }) : <p className="p-8 text-center text-sm text-slate-500">{query.trim() ? "未找到匹配的插件。" : "暂无可安装或可更新的插件。"}</p>}</div></section></>;
}
