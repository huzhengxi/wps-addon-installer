...
export function SearchInput({ value, onChange, placeholder, labelClassName = "", inputClassName = "" }: { value: string; onChange: (value: string) => void; placeholder: string; labelClassName?: string; inputClassName?: string }) {
  ...
  const inputClasses = ["w-full", "rounded-[10px]", "border", "border-slate-200", "bg-white", "pr-3", "pl-10", "text-sm", "shadow-sm", "shadow-slate-900/[0.02]", "outline-none", "transition", "focus:border-brand-400", "dark:border-slate-700", "dark:bg-slate-900", inputClassName].filter(Boolean).join(" ");
