...
export function AddonsPage({ installed, available, query, selected, environment, environmentError, isUninstalling, isRefreshing, installingAddonId, setQuery, setSelected, install, onRefresh, onOpenPermissions, onUninstall }: {
  ...
  return <>
    <PageHeader
      ...
      action={<button type="button" onClick={onRefresh} disabled={isRefreshing || Boolean(installingAddonId) || isUninstalling} aria-busy={isRefreshing} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm font-bold text-slate-700 transition hover:border-brand-200 hover:text-brand-700 disabled:cursor-wait disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"><Spinner size={18} spin={isRefreshing} />{isRefreshing ? "刷新中…" : "刷新"}</button>}
    />
